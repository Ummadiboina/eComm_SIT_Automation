Feature: 27_Verify_campaigns_Acquisition_Bulk_update_channel_for_existing_campaign_and_save_mapped_tariffs

  This scenario ensures that the campaign is mapped to the different tariffs and override channel value of each tariff
  with selected channel in bulk update dropdown, then the customer should be displayed with the campaign on the selected channel for the selected tariffs

  @Web
  Scenario Outline:27_Verify_campaigns_Acquisition_Bulk_update_channel_for_existing_campaign_and_save_mapped_tariffs
    ...
    Examples:
      ...
      | DuraiArasi | DuraiArasi | Shop         | Apple         | Campaigns   | iPad Pro 10.5 inch | Map Tariffs    | Acquisition     |
