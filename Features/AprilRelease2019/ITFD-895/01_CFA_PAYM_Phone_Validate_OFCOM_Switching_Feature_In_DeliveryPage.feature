Feature: 01_CFA_PAYM_Phone_Validate_OFCOM_Switching_Feature_In_DeliveryPage
  ...
  Scenario Outline:01_CFA_PAYM_Phone_Validate_OFCOM_Switching_Feature_In_DeliveryPage
    ...
    And Validate OFCOM switching functionality in consumer channel when ofCom status is <ofComStatus> and <MBBDevice> selected
    ...
    Examples:
      | handset   | Firstname | Surname | Username     | GDPRstatus | MBBDevice | ofComStatus | pacStacCheck | pacStackRetainCheck | ofComMobileNum | pacStacCode  | BSCstatus | BillCap   | CapAmount | B1     | B2     | B3  | B4  | DeviceType | PreSelected | KeyEvent |
