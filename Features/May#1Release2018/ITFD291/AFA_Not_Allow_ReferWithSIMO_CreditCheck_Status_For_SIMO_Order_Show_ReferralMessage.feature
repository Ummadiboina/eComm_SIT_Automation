Feature: AFA_Not_Allow_ReferWithSIMO_Credit_Check_Status_For_SIMO_Order_And_Show_ReferralMessage
  ...
  Scenario Outline: AFA_update_copy_for_Refer_with_Simo_response
    ...
    Examples:
      | Device              | Tariff | DeliveryType | Firstname | Surname          | Username     | HouseNumber | PostCode | B1     | B2  | B3     | B4     | Text   | Email  | Phone | Post | Consumer     | status  | MBBStatus |
      | Galaxy S8 Plus 64GB | SimO   | HomeDelivery | TEST      | ReferralwithSimo | TEST ACCEPTA | 6           | SL11ER   | Select | Not | Select | Select | Select | Select | Not   | Not  | SomeOne else | Enabled | No        |
