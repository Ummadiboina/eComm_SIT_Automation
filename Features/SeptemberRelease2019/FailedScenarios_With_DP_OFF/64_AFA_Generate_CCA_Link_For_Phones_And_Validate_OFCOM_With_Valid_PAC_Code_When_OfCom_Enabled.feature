Feature: 64_AFA_Generate_CCA_Link_For_Phones_And_Validate_OFCOM_With_Valid_PAC_Code_When_OfCom_Enabled
  ...
  Scenario Outline: 64_AFA_Generate_CCA_Link_For_Phones_And_Validate_OFCOM_With_Valid_PAC_Code_When_OfCom_Enabled
    ...
    Examples:
      ...
      | iPhone 7 Plus 128GB Black | Refresh | Enabled   | £60           | HomeDelivery | TEST      | ACCEPTA | TEST ACCEPTA | 14          | SL11UP   | test123  | test123         | vinudeep       | Select | Select | Select | Not | Select | Select | Select | Select | Me       | Enabled    | Disabled    | ink_jun8829@stf.ref.o2.co.uk | test123  | Connected  | Phone        | Yes         | No       | CFA     | CCA      | Enabled     | Yes          | Yes                 | 07707003809    | XBA390304   | valid      | PAC         |
