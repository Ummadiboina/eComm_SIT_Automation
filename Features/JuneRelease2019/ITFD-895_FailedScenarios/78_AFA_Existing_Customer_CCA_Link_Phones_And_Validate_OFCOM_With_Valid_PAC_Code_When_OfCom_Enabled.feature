Feature: 78_AFA_Existing_Customer_CCA_Link_Phones_And_Validate_OFCOM_With_Valid_PAC_Code_When_OfCom_Enabled
  ...
  Scenario Outline: 78_AFA_Existing_Customer_CCA_Link_Phones_And_Validate_OFCOM_With_Valid_PAC_Code_When_OfCom_Enabled
    ...
    Examples:
      | User        | Device                        | Tariffs | Extras    | DeliveryType | BSCstatus | BillCapAmount | username                      | password | Username     | Password | confirmPassword | SecurityAnswer | Consumer | B1     | B2     | B3     | B4  | Text   | Email  | Phone | Post | status  | GDPRstatus2 | KeyEvent | PreSelected | journey | journey2 | DeviceType | DeviceModule | ofComStatus | pacStacCheck | ofComMobileNum | codeStatus | codeVariant | pacStacCode | pacStackRetainCheck |
      | 07521138223 | iPhone 8 Plus 64GB Space Grey | Refresh | Insurance | HomeDelivery | Enabled   | £30           | dono34204837@stf.ref.o2.co.uk | test123  | TEST ACCEPTA | test1234 | test1234        | vinudeep       | Me       | Select | Select | Select | Not | Select | Select | Not   | Not  | Enabled | Disabled    | No       | No          | CFA     | CCA      | Connected  | Phone        | Enabled     | Yes          | 07707003214    | valid      | PAC         | XBA196160   | Yes                 |
