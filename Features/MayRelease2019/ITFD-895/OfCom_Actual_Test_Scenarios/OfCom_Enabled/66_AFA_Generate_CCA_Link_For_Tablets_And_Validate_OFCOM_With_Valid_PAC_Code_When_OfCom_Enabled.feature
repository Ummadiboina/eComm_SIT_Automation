Feature: 66_AFA_Generate_CCA_Link_For_Tablets_And_Validate_OFCOM_With_Valid_PAC_Code_When_OfCom_Enabled

  @Web
  Scenario Outline: 66_AFA_Generate_CCA_Link_For_Tablets_And_Validate_OFCOM_With_Valid_PAC_Code_When_OfCom_Enabled
    ...
    Examples:
      ...
      | iPad Pro 10.5 inch 256GB Space Grey | Refresh | Enabled   | £60           | HomeDelivery | TEST      | ACCEPTA | TEST ACCEPTA | 14          | SL11UP   | test123  | test123         | vinudeep       | Select | Select | Select | Not | Select | Select | Select | Select | Me       | Enabled    | Disabled    | ink_jun8829@stf.ref.o2.co.uk | test123  | Connected  | Phone        | Yes         | No       | AFA     | CFA      | Enabled     | Yes          | Yes                 | 07123456789    | 123456JKKhan | valid      | PAC         |
