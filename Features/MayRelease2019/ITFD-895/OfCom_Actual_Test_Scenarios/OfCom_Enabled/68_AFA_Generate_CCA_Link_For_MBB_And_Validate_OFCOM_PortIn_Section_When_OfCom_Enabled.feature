Feature: 68_AFA_Generate_CCA_Link_For_MBB_And_Validate_OFCOM_PortIn_Section_When_OfCom_Enabled
  ...
  Scenario Outline: 68_AFA_Generate_CCA_Link_For_MBB_And_Validate_OFCOM_PortIn_Section_When_OfCom_Enabled
    ...
    Examples:
      ...
      | dongle | Refresh | Enabled   | £10           | HomeDelivery | TEST      | ACCEPTA | TEST ACCEPTA | 14          | SL11UP   | test123  | test123         | vinudeep       | Select | Select | Select | Not | Select | Select | Select | Select | Me       | Enabled    | Disabled    | ink_jun8829@stf.ref.o2.co.uk | test123  | Connected  | Phone        | Yes         | No       | MBB     | Enabled     | Yes          | Yes                 | 07123456789    | 123456JKKhan | valid      | PAC         |
