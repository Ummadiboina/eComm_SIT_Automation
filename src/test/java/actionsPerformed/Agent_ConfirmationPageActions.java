			...
			if(ofComStatus.equalsIgnoreCase("Enabled")) {
				...
				if (journey.equalsIgnoreCase("PayGsimo") || journey.equalsIgnoreCase("Tablet") || journey.equalsIgnoreCase("MBB") || journey.equalsIgnoreCase("AFU") || journey.equalsIgnoreCase("Accessory")) {
					...
					if (driver.findElements(By.xpath("//div[@id='msisdn-confirmation-msg-id']")).size() > 0) {
						...
						String selectedStatusMessage = Agent_ConfirmationPage.ofComStatusMsg.getText();
						String ofComStatusMsgCustomerNewNum = Agent_ConfirmationPage.ofComStatusMsgCustomerNewNum.getText();
						Thread.sleep(1000);
						...
						if (PacStacCheck.equalsIgnoreCase("Yes")) {
						...
						} else if (PacStacCheck.equalsIgnoreCase("Not")) {
							...
							Thread.sleep(3000);
							if (selectedStatusMessage.contains(expectedStatusMessage)) {
								log.debug("OFCOM switching status is matching when portIn is not opted ie: " + selectedStatusMessage + "\n");
							} else {
								log.debug("Failed: OFCOM switching status is not matching when portIn is not opted ie: " + selectedStatusMessage + "\n");
								Assert.fail("Failed: OFCOM switching status is not matching when portIn is not opted ie: " + selectedStatusMessage + "\n");
							}
						}
					} else {
