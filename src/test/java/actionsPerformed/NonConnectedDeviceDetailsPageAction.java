...
public class NonConnectedDeviceDetailsPageAction extends Environment {
	...
	public static void capacitySelectOfDeviceDropDown(String capacity) throws Exception {
		...
		WebElement element = pageobjects.NonConnectedDeviceDetailsPage.CapacityDropDown;
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].setAttribute('style', 'display:block;')", element);
		if (element.isDisplayed()) {
	...
	public static void checkOnlyOneColourAvailable() throws Exception {
		// TODO Auto-generated method stub
		Thread.sleep(5000);
		List<org.openqa.selenium.WebElement> ColorLabel = pageobjects.NonConnectedDeviceDetailsPage.ColorLabel;
		System.out.println("color label size :" + ColorLabel.size());
		if (ColorLabel.size() > 1) {
			Assert.fail("There are more than 1 color variant displayed as a label");
		}
		JavascriptExecutor js = (JavascriptExecutor) driver;
